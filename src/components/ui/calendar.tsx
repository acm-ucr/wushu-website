...
function Day(
  ...
  const { day, events, setCurrent } = props;
  const currentMonth = new Date().getMonth() === day.date.getMonth();
  const isToday = new Date().toDateString() === day.date.toDateString();

  return (
    <div
      className={cn(
        "relative flex size-13 flex-col justify-start p-1 md:size-24 lg:size-36",
        currentMonth ? "text-black" : "text-gray-400",
        isToday ? "bg-accent text-accent-foreground" : "hover:bg-white",
        "scrollbar-hidden overflow-y-auto hover:bg-white",
      )}
    >
      <span className="text-right font-normal md:pr-1 md:text-lg lg:p-0.5 lg:text-2xl">
        {day.date.getDate()}
      </span>
          ...
          return (
            ...
            >
              {startDate.getHours() < 12
                ? (startDate.getHours() % 12 || 12) + "am"
                : (startDate.getHours() % 12 || 12) + "pm"}{" "}
              {title}
            </div>
          );
        }
      ...
      <DayPicker
        ...
        classNames={{
          ...
          button_previous: cn(
            ...
            "size-(--cell-size) -translate-y-4 aria-disabled:opacity-50 p-0 select-none",
          ...
          caption_label: cn(
            "select-none font-medium text-2xl md:text-4xl lg:text-6xl lg: mb-6 font-semibold font-khula text-wushu-red-100",
            captionLayout === "label"
              ? "text-2xl"
          ...
          day: cn(
            "relative w-full h-full p-0 text-center border border-wushu-red-100 [&:first-child[data-selected=true]_button]:rounded-l-md [&:last-child[data-selected=true]_button]:rounded-r-md group/day aspect-square select-none",
        ...
        components={{
          ...
          Chevron: ({ className, orientation, ...props }) => {
            if (orientation === "left") {
              return (
                <ChevronLeftIcon
                  className={cn("lg:size-10", className)}
                  {...props}
                />
              );
            }
          ...
          DayButton: (props) => (
            <Day {...props} events={events} setCurrent={setCurrent} />
          ),
...
export { CalendarUI, CalendarDayButton };
